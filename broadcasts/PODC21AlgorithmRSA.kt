...
import crypto.RSA.Companion.sign
import crypto.RSA.Companion.verify
import udp.Utils
import java.io.File
...
class PODC21AlgorithmRSA(
    ...
    private fun broadcast(message: Message) {
        val sign = sign(message, pair.private)
    ...
    private fun vote(from: HostName, message: Message, sign: String) {
        ...
        if (verify(message, pair.public, signDecoded)) {
        ...
        for ((_, msg) in votes) {
            ...
            if (msg != "" && messageCnt[msg]!! >= (n - f) && !delivered) {
                ...
                FileOutputStream(
                    File("$basePath/rsa_${n}_${f}_${port - sourcePort + 9000}"),
                    true
